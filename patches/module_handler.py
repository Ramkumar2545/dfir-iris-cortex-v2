...
#   task_hook_wrapper: guard data encoding so that bytes payloads (or data
#   already in bytes form due to upstream codec issues) don't raise
#   "TypeError: encoding without a string argument".
#   Also uses split(b" ", 1) to tolerate base64 padding that contains spaces.
import traceback
...
def task_hook_wrapper(self, module_name, hook_name, hook_ui_name, data, init_user, caseid):
    try:
        # FIX: guard against bytes or non-str data arriving from the queue.
        # Use split(b" ", 1) to handle any space inside base64 padding safely.
        if isinstance(data, bytes):
            raw = data
        else:
            raw = str(data).encode("utf-8")
        signature, pdata = raw.split(b" ", 1)
