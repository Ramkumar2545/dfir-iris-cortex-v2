...
#   setup_periodic_update_checks: renamed first arg from 'self' to 'sender=None'
#   so Blinker's keyword-only dispatch works without raising
...
def call_ext_updater(update_archive, scope, need_reboot):
    ...
    try:
        ...
        subprocess.Popen(["/bin/bash", f"{source_dir}/iris_updater.sh",
                          update_archive.as_posix(),
                          target_dir.as_posix(),
                          archive_name,
                          scope[0],
                          '1' if docker else '0',
                          '1' if need_reboot else '0',
                          '&'])
...
# FIX: renamed first arg from 'self' to 'sender=None'.
# Blinker dispatches on_after_finalize as a keyword arg (sender=<app>),
# so a positional-only 'self' param never gets populated → TypeError.
@celery.on_after_finalize.connect
def setup_periodic_update_checks(sender=None, **kwargs):
