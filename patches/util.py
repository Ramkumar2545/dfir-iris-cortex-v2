...
# Source: dfir-iris/iris-web tag v2.4.27 — source/app/util.py (SHA 4bdff48f)
# Change: added _secret_key_bytes() helper; updated hmac_sign + hmac_verify
#         to use it instead of bare bytes(SECRET_KEY, "utf-8").
#
# Root cause: v2.4.27 config migration stores SECRET_KEY as bytes under
# IRIS_SECRET_KEY.  app.config.get("SECRET_KEY") returns bytes (or None),
# and bytes(bytes_val, "utf-8") raises:
#     TypeError: encoding without a string argument
# The helper handles str, bytes, and None safely.
# ──────────────────────────────────────────────────────────────────────────────
import base64
import datetime
import decimal
...
# ── PATCHED: _secret_key_bytes() helper ───────────────────────────────────────────────
def _secret_key_bytes():
    """
    Return SECRET_KEY as raw bytes regardless of its stored type.
    Bypasses Flask app.config to prevent Celery ForkPoolWorkers from
    evaluating the key as None when the app context drops.
    """
    import os
    # Read directly from the OS environment to guarantee App and Worker match perfectly
    key_val = os.environ.get("IRIS_SECRET_KEY") or os.environ.get("SECRET_KEY") or app.config.get("SECRET_KEY")
...
def hmac_sign(data):
    key = _secret_key_bytes()          # PATCHED: was bytes(app.config.get("SECRET_KEY"), "utf-8")
